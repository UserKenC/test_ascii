...
void exit(void)
{
  ...
  curproc->turnaround_time = ticks - curproc->creation_time;

  // Print scheduling statistics
  cprintf("Process %d exited: wait_time=%d, run_time=%d, turnaround_time=%d\n",
          curproc->pid, curproc->wait_time, curproc->run_time,
...
// PAGEBREAK: 42
//  Per-CPU process scheduler with priority support.
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    // UPDATE ALL PROCESS TIMING FIRST
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    {
      if (p->state != UNUSED && p->last_scheduled != 0)
      {
        if (p->state == RUNNING)
        {
          p->run_time += ticks - p->last_scheduled;
        }
    ...
    if (chosen != 0)
    {
      // Set last_scheduled for the chosen process
      chosen->last_scheduled = ticks;
      ...
      // Update run time after context switch
      if (chosen->last_scheduled != 0)
      {
        chosen->run_time += ticks - chosen->last_scheduled;
      }
      chosen->last_scheduled = ticks;
...
void sleep(void *chan, struct spinlock *lk)
{
  ...
  // Priority donation: if we're sleeping on a lock, check if holder has lower
  // priority
  if (chan != 0)
  {
    struct proc *holder = 0;
    // This is simplified - in real implementation you'd track lock holders
    // For demonstration, we'll check if any process is holding this lock
    // and has lower priority than us
  }
...
void procdump(void)
{
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    ...
    if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
    cprintf("%d %s %s prio:%d wait:%d run:%d", p->pid, state, p->name,
            p->priority, p->wait_time, p->run_time);
    if (p->state == SLEEPING)
    {
