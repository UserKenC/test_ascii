...
int main(int argc, char *argv[]){
    if (argc != 4){
        fprintf(stderr, "Usage: %s <shmid> <semid> <producer_id>\n", argv[0]);
        exit(EXIT_FAILURE);
    }
    int shmid = atoi(argv[1]);
    int semid = atoi(argv[2]);
    int pid_id = atoi(argv[3]);

    srand(time(NULL) ^ (getpid()<<16));

    shm_buf_t *shmp = (shm_buf_t*) shmat(shmid, NULL, 0);
    if (shmp == (void*)-1) perror_exit("producer shmat");

    for (int iter=0; iter<6; iter++){
        int t = (rand() % 3) + 1;
        sleep(t);

        if (semop(semid, (struct sembuf[]){ P(1) }, 1) == -1) perror_exit("semop P empty");
        if (semop(semid, (struct sembuf[]){ P(0) }, 1) == -1) perror_exit("semop P mutex");

        int item = shmp->next_item_id++;
        ...
        print_time();
        printf("Producer %d produced item %d into slot %d | buffer=[", pid_id, item, pos);
        ...
        if (semop(semid, (struct sembuf[]){ V(2) }, 1) == -1) perror_exit("semop V full");
    }

    if (shmdt(shmp) == -1) perror_exit("producer shmdt");
    return 0;
}
